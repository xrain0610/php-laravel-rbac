			...
			<div class="ibox-content"> 
				<form method="post" class="form-horizontal">
					<div class="form-group"><label class="col-sm-2 control-label">{{trans('common.name')}}</label>
						<div class="col-sm-7"><input type="text" name="name" value="{{$role->name}}" class="form-control"></div>
					...
					<div class="form-group"><label class="col-sm-2 control-label">{{trans('common.desc')}}</label>
						<div class="col-sm-7"><input type="text" name="desc" value="{{$role->desc}}" class="form-control"></div>
						@if (\Session::has('errors'))
					...
					<div class="form-group"><label class="col-sm-2 control-label">{{$p['name']}}</label>
                        ...
                        @foreach ($p['list'] as $subp)
                     	<label class="checkbox-inline" title="{{$subp['desc']}}"><input type="checkbox" name="permissions[]" @if (in_array($subp['id'],explode('|',$role->permissions))) checked="" @endif value="{{$subp['id']}}" id="inlineCheckbox{{$subp['id']}}">{{$subp['name']}}</label> 
                        @endforeach
                        </div>
                    </div>
                    <div class="hr-line-dashed"></div>
